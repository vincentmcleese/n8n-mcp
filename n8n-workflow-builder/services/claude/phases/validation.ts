...
export class ValidationPhaseService extends BasePhaseService<ValidationInput, ValidationOutput> {
  ...
  async execute(
    ...
    try {
      ...
      if (result.data.workflow && typeof result.data.workflow.valid === 'undefined') {
  ...
  async generateValidationFixes(
    ...
    try {
      ...
      const promptParts = ValidationPrompts.getValidationFixesPrompt(errors, workflow);
      ...
      if (!result.success || !result.data) {
        return {
          success: false,
          error: result.error || new Error('Failed to generate validation fixes'),
          usage: result.usage,
        };
      }
      
      this.logSuccess('Validation fixes generated', {
        fixCount: result.data.fixes?.length || 0,
        errorCount: errors.length,
      });
   ...
   */
  private async callClaudeForValidation(draftWorkflow: any): Promise<PhaseResult<ValidationOutput>> {
    ...
    try {
      ...
      const markerMatch = text.match(/=== BEGIN RESULT ===\s*([\s\S]*?)\s*=== END RESULT ===/);
      
      if (!markerMatch) {
        this.logError('callClaudeForValidation', 'No result markers found in response');
      ...
      try {
        ...
        const validationResult = validatedWorkflowResponseSchema.safeParse(result);
        if (!validationResult.success) {
          this.logError('callClaudeForValidation', `Schema validation failed: ${validationResult.error}`);
        ...
        this.logError('callClaudeForValidation', `Failed to parse JSON: ${parseError}`);
  ...
  private countValidationErrors(report: any): number {
    ...
    if (report.workflow?.errors) {
      count += Array.isArray(report.workflow.errors) ? report.workflow.errors.length : 0;
    }
    
    if (report.initial?.workflow?.errors) {
      count += Array.isArray(report.initial.workflow.errors) ? report.initial.workflow.errors.length : 0;
    }
    
    if (report.final?.workflow?.errors) {
      count += Array.isArray(report.final.workflow.errors) ? report.final.workflow.errors.length : 0;
    }
