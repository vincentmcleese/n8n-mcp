...
export class DocumentationPhaseService extends BasePhaseService<DocumentationInput, DocumentationOutput> {
  ...
  async execute(
    ...
    try {
      ...
      const promptParts = DocumentationPrompts.getDocumentationPrompt(userPrompt, workflow, nodeMetadata);
      ...
      this.logSuccess('Documentation generation', {
        ...
        stickyNotes: validNotes.filter(op => op.type === 'addStickyNote').length,
      });
      ...
      return {
        ...
        data: {
          ...
          reasoning: result.data.reasoning || ['Generated workflow documentation'],
          usage: result.usage,
        },
        usage: result.usage,
   ...
   */
  private validateStickyNotes(operations: WorkflowOperation[]): WorkflowOperation[] {
    return operations.filter(op => {
      ...
      if (!note) {
        this.logWarning('validateStickyNotes', 'Sticky note operation missing note object');
        return false;
      }
      
      // Validate required fields
      if (!note.id || !note.content || !Array.isArray(note.nodeGroupIds)) {
        this.logWarning('validateStickyNotes', `Invalid sticky note structure: ${JSON.stringify(note)}`);
        return false;
      }
      
      // Validate content starts with markdown title
      if (!note.content.startsWith('##')) {
        this.logWarning('validateStickyNotes', 'Sticky note content must start with ## for markdown title');
        // Try to fix it
      ...
      if (note.color && (note.color < 1 || note.color > 7)) {
        this.logWarning('validateStickyNotes', `Invalid color ${note.color}, using default`);
  ...
  private determineColorFromContent(content: string): number {
    ...
    if (lowerContent.includes('error') || lowerContent.includes('warning') || lowerContent.includes('critical')) {
    ...
    if (lowerContent.includes('api') || lowerContent.includes('integration') || lowerContent.includes('external')) {
      return 5; // Cyan for integrations
    }
    
    if (lowerContent.includes('ai') || lowerContent.includes('ml') || lowerContent.includes('model')) {
      return 6; // Purple for AI/ML
    }
    
    if (lowerContent.includes('config') || lowerContent.includes('setup') || lowerContent.includes('initial')) {
      return 7; // Orange for configuration
    }
    
    if (lowerContent.includes('data') || lowerContent.includes('process') || lowerContent.includes('transform')) {
   ...
   */
  private generateNodeGroupings(nodeMetadata: NodeMetadata[]): Map<string, NodeMetadata[]> {
    ...
    for (const node of nodeMetadata) {
      ...
      if (type.includes('trigger') || type.includes('webhook') || type.includes('schedule')) {
        triggers.push(node);
      } else if (type.includes('function') || type.includes('code') || type.includes('transform')) {
        dataProcessing.push(node);
      } else if (type.includes('http') || type.includes('api') || type.includes('request')) {
        integrations.push(node);
      } else if (type.includes('slack') || type.includes('email') || type.includes('respond')) {
        outputs.push(node);
      } else {
        // Default grouping
        dataProcessing.push(node);
