...
import { retryWithBackoff, DEFAULT_DB_RETRY_OPTIONS } from './db/transaction-utils';
import type { 
  WorkflowSession, 
  WorkflowOperation,
  WorkflowPhase 
  ...
  try {
    ...
    for (const op of operations) {
      switch (op.type) {
        ...
        case 'discoverNode':
          ...
          if (op.node && !newState.discovered.find(n => n.id === op.node.id)) {
        ...
        case 'deselectNode':
          ...
          if (op.nodeId) {
            newState.selected = newState.selected.filter(n => n !== op.nodeId);
          }
          break;
    ...
    if (updateError) {
      ...
      if (updateError.code === 'PGRST116') { // No rows returned = version mismatch
        return { success: false, error: 'Version conflict - another update occurred' };
      }
