 ...
 */
const ERROR_MAPPINGS: Record<string, { userMessage: string; retryable: boolean }> = {
  // Claude API errors
  ECONNREFUSED: {
...
export function wrapPhase<TInput, TOutput extends PhaseResult<any>>(
  ...
  return async function wrappedRun(
    ...
    try {
      loggers.orchestrator.info(`Starting ${phaseName} phase for session ${sessionId}`);
...
function createErrorResponse(error: PhaseError): ErrorResponse["error"] {
  ...
  return {
    ...
    retryable: error.retryable !== undefined ? error.retryable : errorMapping.retryable,
  };
}
 ...
 */
function determineErrorType(error: PhaseError): string {
...
function extractErrorCode(error: PhaseError): string {
  ...
  const codeMatch = error.message.match(/\b([A-Z_]+_ERROR|[A-Z_]+_FAILED|NO_[A-Z_]+)\b/);
  if (codeMatch) {
    return codeMatch[1];
  }
